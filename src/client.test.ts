...
const variant_name = Math.random().toString(36).slice(2);

test('insert', async () => {
	const result =
		await mysqlClient.sql<ResultSetHeader>`INSERT INTO variant_ids ${bunSql({ variant_name })}`;
...
test('delete', async () => {
	const result =
		await mysqlClient.sql<ResultSetHeader>`DELETE FROM variant_ids WHERE variant_name = ${variant_name}`;
